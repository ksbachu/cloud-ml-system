...
resource "aws_appautoscaling_target" "sagemaker_autoscaling_target" {
  max_capacity       = 10
  min_capacity       = 1
  resource_id        = "endpoint/${aws_sagemaker_endpoint.xgboost_endpoint.name}/variant/AllTraffic"
  scalable_dimension = "sagemaker:variant:DesiredInstanceCount"
  service_namespace  = "sagemaker"
}

resource "aws_appautoscaling_policy" "sagemaker_autoscaling_policy" {
  name               = "sagemaker-invocation-scaling-policy"
  ...
  target_tracking_scaling_policy_configuration {
    target_value       = 30.0 # Adjust based on concurrency needs
